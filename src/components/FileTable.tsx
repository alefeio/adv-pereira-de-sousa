...
export default function FileTable({ files, handleFileClick, handleEditFile, handleDelete, confirmingDeleteId, setConfirmingDeleteId, getFileIcon }: Props) {
  return (
    <div className="bg-white shadow-lg rounded-lg overflow-hidden">
      {files.length === 0 ? (
        <p className="p-6 text-white text-center">Nenhum arquivo encontrado.</p>
      ) : (
        <div className="overflow-x-auto">
          <table className="min-w-full divide-y divide-gray-200">
            <thead className="bg-gray-50">
              <tr>
                <th className="px-6 py-3 text-left text-xs font-medium text-white uppercase tracking-wider">Arquivo</th>
                <th className="px-6 py-3 text-left text-xs font-medium text-white uppercase tracking-wider">Projeto</th>
                <th className="px-6 py-3 text-left text-xs font-medium text-white uppercase tracking-wider">Tarefa</th>
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {files.map(file => (
                <tr key={file.id} className="hover:bg-gray-100 transition duration-150">
                  ...
                  <td className="px-6 py-4 text-sm text-white">{file.projeto?.title || 'N/A'}</td>
                  <td className="px-6 py-4 text-sm text-white">{file.task?.title || 'N/A'}</td>
                  <td className="px-6 py-4 text-sm text-white">{file.mimetype}</td>
                  <td className="px-6 py-4 text-sm text-white">{new Date(file.createdAt).toLocaleDateString()}</td>
                  <td className="px-6 py-4 text-right text-sm font-medium flex gap-2 justify-end">
                    ...
                    {confirmingDeleteId === file.id ? (
                      <>
                        ...
                        <button onClick={() => setConfirmingDeleteId(null)} className="bg-white hover:bg-white text-white px-3 py-1 rounded-md">Cancelar</button>
                      </>
                    ) : (
